...
Теперь надо запустить Linux. Для загрузки Linux необходимо ввести следующие команды:
```
set root=(hd1,gpt2)
...
Находим что-то похожее на `root` и переписываем команду и запускаем систему:
```
set root=(hd1,gpt2)
